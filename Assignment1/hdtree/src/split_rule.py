...
class AbstractCategoricalSplit(AbstractSplitRule):
    ...
    def _get_attribute_candidates(self) -> typing.List[int]:
        ...
        """
        candidates = []
        attr_types = self.get_tree().get_attribute_types()[:-1]
        # print('attr', attr_types, type(attr_types))
...
class TwoAttributeSplitMixin(AbstractSplitRule):
            ...
            for j in range(i + 1, len(supported_cols)):
                ...
                if np.any(non_null_indexer_1_2):
                    ...
                    rets = self._get_children_indexer_and_state(col_data_1[non_null_indexer_1_2],
                                                                col_data_2[non_null_indexer_1_2], tgt_data[non_null_indexer_1_2])
    ...
    @abstractmethod
    def _get_children_indexer_and_state(self, data_values_left: np.ndarray, data_values_right: np.ndarray, data_values_tgt: np.ndarray, *args,
                                        **kwargs) -> Optional[
    ...
    # def get_split_attribute_indices(self) -> typing.Dict:
        """
...
class OneAttributeSplitMixin(AbstractSplitRule):
    ...
    def _get_best_split(self):
        ...
        supported_cols = self._get_attribute_candidates()
        supported_cols = [i for i in supported_cols if i != node_data.shape[1] - 1]
        # print(node_data.shape, supported_cols)
        # print(node_data, supported_cols)
        ...
        for attr_idx in supported_cols:
            ...
            tgt_data = node_data[:, -1]
            # print('col', len(col_data), node_data.shape)
            ...
            if np.any(non_null_indexer):
                ...
                non_null_tgts = tgt_data[non_null_indexer]
                # get indexer for child node data assignments
                # print('type', type(non_null_values), type(tgt_data))
                rets = self._get_children_indexer_and_state(non_null_values, non_null_tgts, attr_idx)
...
class AbstractRangeSplit(AbstractNumericalSplit, OneAttributeSplitMixin):
    ...
    def get_child_node_indices_for_sample(self, sample: np.ndarray) -> typing.List['Node']:
        ...
        if val is None:
            ...
            if self.sigmoid(val*lower + upper) > 0.5:
                return [0]
            else:
                return [1]
    ...
    def get_edge_labels(self):
        return ["sigmoid <= 0.5", "sigmoid > 0.5"]
    ...
    def user_readable_description(self):
        ...
        if self.is_initialized():
            ...
            return f"logistic rergression on {attr_name}"
        else:
...
@hide_in_ui
class LogisticRegressionAbstractQuantileRangeSplit(AbstractRangeSplit):
    """
    Will divide the data into Quantiles and splits data laying inside a span of these or outside
    """
    ...
    def _get_children_indexer_and_state(self, data_values: np.ndarray, target_values: np.ndarray, *args, **kwargs):
        # print(target_values)
        # print('data', data_values.shape, target_values.shape)

        nodeLR = LogisticRegression()
        nodeLR.fit(data_values.reshape(-1, 1), target_values)
        probabs = nodeLR.predict_proba(data_values.reshape(-1, 1))
        probabs = np.argmax(probabs, axis=1)

        # print('size', data_values.shape, probabs.shape)

        # left = probabs == 0
        # right = np.ndarray(probabs == 1)
        # left = np.array(left)
        # for i in probabs
        ...
        # quantile_vals = np.quantile(data_values, quantile_parts)
        # quantile_vals = list(set(list(quantile_vals)))  # this will remove doubled entries
        # print('quant', quantile_vals, quantile_parts)
        # results = [({'lower_bound': 0.5,
                    #  'upper_bound': 1},
                    # (left, ~left))]
        ...
        results = []
        leftProbs = (probabs == 1)
        # print('left', leftProbs.shape)

        # left = [data_values if probabs == 0]
        # right = [data_values if probabs == 1]
        # print('int', nodeLR.coef_, nodeLR.intercept_)
        results.append(({'lower_bound': nodeLR.coef_[0][0],
                         'upper_bound': nodeLR.intercept_[0]},
                        (leftProbs, ~leftProbs)))
        ...
        #                         (left, ~left)))
        #         # print('hi')
        #         print(type(left), left.shape, left)
...
class TwoQuantileRangeSplit(LogisticRegressionAbstractQuantileRangeSplit):
    @property
    def quantile_count(self) -> int:
        return 2
...
@hide_in_ui
class LogisticRegressionDoubleCategorySplit(AbstractNumericalSplit, TwoAttributeSplitMixin):
    """
    Splits on a1 < a2 rule
    """
    ...
    def get_child_node_indices_for_sample(self, sample: np.ndarray) -> typing.List['Node']:
        ...
        state = self.get_state()
        attr_idx_1, attr_idx_2 = self.get_split_attribute_indices()
        # attr_idx_1, attr_idx_2 = state['split_attribute_indices']
        coef0 = state['coef0']
        coef1 = state['coef1']
        ...
        # print('c', self._coef0, self._coef1, self._intercept)
        if attr1 is None or attr2 is None:
            return [0, 1]
        else:
            # if self._coef0 is not None:
            if self.sigmoid(coef0 * attr1 + coef1 * attr2 + intercept) > 0.5:
                return [0]
            else:
                return [1]
            # else:
    ...
    def _get_children_indexer_and_state(self, data_values_left: np.ndarray, data_values_right: np.ndarray, data_values_tgt: np.ndarray, *args,
                                        **kwargs):
        data = np.stack((data_values_left, data_values_right), axis=-1)
        nodeLR = LogisticRegression()
        nodeLR.fit(data, data_values_tgt)
        probabs = nodeLR.predict_proba(data)
        probabs = np.argmax(probabs, axis=1)
        self.coef0 = nodeLR.coef_[0][0]
        self.coef1 = nodeLR.coef_[0][1]
        self.intercept = nodeLR.intercept_[0]
        # print('c', self.coef, self.intercept)
        # print('data_values_left', data_values_left.shape, data_values_right.shape, data_values_tgt.shape, data.shape)
        # left_vals = data_values_left < data_values_right
        left_vals = (probabs == 1)
        state = {
                    'coef0': nodeLR.coef_[0][0], 
                    'coef1': nodeLR.coef_[0][1], 
                    'intercept': nodeLR.intercept_[0]
                }
...
@hide_in_ui
class LogisticRegressionSingleSplit(AbstractCategoricalSplit, OneAttributeSplitMixin):
    """
    Will split on a single categorical attribute
    e.g. an attribute having 10 unique values will split in up tp 10 childs
    (Depending if node in question has all values inside)
...
@hide_in_ui
class LogisticRegressionSplit(AbstractNumericalSplit, TwoAttributeSplitMixin):
    """
    Splits on a1 < 1/2 * a2 rule
    """
